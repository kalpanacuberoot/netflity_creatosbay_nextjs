...
const Creators_profilepage = () => {
  ...
  useEffect(() => {
    ...
    //   // Check if the new URL matches the one you want to trigger a refresh on
    //   if (`${url}/${creator_profile_id}` === `${url}/${creator_profile_id}`) {
    ...
    // // Listen for route changes and call the handleRouteChange function
  ...
  const handleReload = () => {
    ...
    if (hasReloaded !== 'true') { // Check if the page has not been reloaded yet
      // Open the modal just before reloading
      window.location.reload();
      Cookies.set('hasReloaded', 'true'); // Set the flag to indicate that the page has been reloaded
      setIsModalOpen(false);
    }
    else {
      setIsModalOpen(true);
